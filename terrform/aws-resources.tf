...
resource "aws_internet_gateway" "sprints_gateway" {
  vpc_id = aws_vpc.dev.id
...
resource "aws_route_table" "sprints_routet" {
  vpc_id = aws_vpc.dev.id

  route {
    cidr_block = "0.0.0.0/0"
    gateway_id = aws_internet_gateway.sprints_gateway.id
  }
...
resource "aws_route_table_association" "sprints_rta" {
  ...
  route_table_id = aws_route_table.sprints_routet.id
}
